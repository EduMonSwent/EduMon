...
class LevelRewardEngine(private val config: LevelRewardConfig = LevelRewardConfig) {
  ...
  fun applyLevelUpRewards(oldProfile: UserProfile, newProfile: UserProfile): Result {
    // no level increase → nothing to do
    ...
    newlyRewardedLevels.forEach { level ->
      ...
      if (reward.extraStudyTimeMin != 0) {
        val stats = profile.studyStats
        profile =
            profile.copy(
                studyStats =
                    stats.copy(totalTimeMin = stats.totalTimeMin + reward.extraStudyTimeMin))
        extraStudyTime += reward.extraStudyTimeMin
      }
    ...
    val summary =
        GrantedRewardsSummary(
            ...
            extraStudyTimeMinGranted = extraStudyTime)
