...
private fun DocumentSnapshot.toPlannerClass(): Class? {
    ...
    val type =
        runCatching { ClassType.valueOf(typeStr) }.getOrNull()
            ?: ClassType.LECTURE
...
private fun DocumentSnapshot.toClassAttendance(): ClassAttendance? {
    ...
    val attendance =
        runCatching { AttendanceStatus.valueOf(attendanceStr) }.getOrNull()
            ?: AttendanceStatus.YES
    val completion =
        runCatching { CompletionStatus.valueOf(completionStr) }.getOrNull()
            ?: CompletionStatus.NO
