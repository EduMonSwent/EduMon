...
/**
 * Firestore-backed implementation that stores objectives under:
 *   users/{uid}/objectives
 *
 * Ordering is maintained by a monotonically increasing `order` field
 * so that index-based operations behave consistently across devices.
 */
class FirestoreObjectivesRepository(
    ...
    private fun Objective.toFs(order: Long?): MutableMap<String, Any?> = hashMapOf(
        "title" to title,
        "course" to course,
    ...
    override suspend fun getObjectives(): List<Objective> = withContext(Dispatchers.IO) {
        fetchOrdered().map { it.second }
    }

    override suspend fun addObjective(obj: Objective): List<Objective> = withContext(Dispatchers.IO) {
        val items = fetchOrdered()
        val nextOrder = items.size.toLong()
        val data = obj.toFs(order = nextOrder).apply {
        ...
        withContext(Dispatchers.IO) {
            ...
            Tasks.await(db.runBatch { b ->
                ...
                objs.forEachIndexed { i, obj ->
                    ...
                    val data = obj.toFs(order = i.toLong()).apply {
                        this["createdAt"] = FieldValue.serverTimestamp()
                    }
                    b.set(ref, data)
                }
