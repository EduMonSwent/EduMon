...
class ProfileViewModelTest {
  ...
  @Test
  fun toggles_and_accent_update_profile() = runTest(dispatcher) {
  ...
  private class TestRepo(initial: UserProfile) : ProfileRepository {
    ...
    override suspend fun updateProfile(newProfile: UserProfile) { state.value = newProfile }
  }

  @Test
  fun unequip_removes_only_that_slot() = runTest(dispatcher) {
    val repo = TestRepo(UserProfile(accessories = listOf("head:halo", "torso:scarf", "legs:boots")))
  ...
  @Test
  fun equip_legs_cleans_legacy_singular_prefix_and_sets_new() = runTest(dispatcher) {
  ...
  @Test
  fun accent_variants_trigger_blend_and_boost_paths() = runTest(dispatcher) {
    val vm = ProfileViewModel()
